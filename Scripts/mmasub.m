...
f0val,df0dx,fval,dfdx,low,upp,a0,a,c,d,move,asyinit);
%
...
%Calculation of asymptotes low and upp with constant move limits
% low = xval - asyinit*(xmax-xmin);
% upp = xval + asyinit*(xmax-xmin);
% Calculation of the asymptotes low and upp hybrid option:
% if iter < 2.5
%   low = xval - asyinit*(xmax-xmin);
%   upp = xval + asyinit*(xmax-xmin);
% else
...
%   low = xval - factor.*(xold1 - low);
%   upp = xval + factor.*(upp - xold1);
%   lowmin = xval - asyinit*(xmax-xmin);
%   lowmax = xval - 0.01*(xmax-xmin);
%   uppmin = xval + 0.01*(xmax-xmin);
%   uppmax = xval + asyinit*(xmax-xmin);
%   low = max(low,lowmin);
%   low = min(low,lowmax);
%   upp = min(upp,uppmax);
%   upp = max(upp,uppmin);
